...
def _auth_patches():
    return [
        patch("app.auth.sso.get_profile_from_session", new=AsyncMock(return_value=PROFILE_ID)),
        patch("app.routes.stats.get_profile_tz", new=AsyncMock(return_value="UTC")),
        patch("app.routes.stats.count", new=AsyncMock(return_value=10)),
        patch("app.routes.stats.daily_counts", new=AsyncMock(return_value=(_LABELS, _DAILY))),
        patch("app.routes.stats.top_items", new=AsyncMock(return_value=_TOP)),
        patch("app.routes.stats.workspace_breakdown", new=AsyncMock(return_value=_TOP)),
        patch("app.routes.stats.week_bounds", return_value=(_NOW, _NOW)),
    ]
...
async def test_github_stats_week_response_shape():
    ...
    assert any("change" in m for m in body["metrics"])
...
async def test_github_stats_today_response_shape():
    ...
    assert not any("change" in m for m in body["metrics"])
