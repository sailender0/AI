...
def test_jira_due_buckets():
    ...
    assigned = {"issues": [
        {"key": "AI-1", "due_date": "2026-07-20"},   # overdue
        {"key": "AI-2", "due_date": "2026-07-22"},   # overdue
        {"key": "AI-3", "due_date": "2026-07-23"},   # due today  -> soon
        {"key": "AI-4", "due_date": "2026-07-25"},   # within 3d  -> soon
        {"key": "AI-5", "due_date": "2026-07-30"},   # far off    -> neither
        {"due_date": None},                          # ignored
        {},                                          # ignored
        {"key": "AI-8", "due_date": "not-a-date"},   # unparseable -> ignored
    ]}
...
def test_shipped_phrase():
    assert _shipped_phrase(0, 0, 0) == ""                      # nothing shipped
    assert _shipped_phrase(1, 0, 0) == "1 commit"             # singular
    assert _shipped_phrase(4, 2, 1) == "4 commits, 2 PR updates, 1 issue update"
    assert _shipped_phrase(0, 0, 3) == "3 issue updates"     # omits zero counts
