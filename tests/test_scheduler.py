...
def test_weekly_only_at_local_friday_17():
    ...
    assert not _is_scheduled_time("weekly", datetime(2026, 7, 3, 16, 5))
    assert not _is_scheduled_time("weekly", THU)
...
def test_standup_only_at_local_09():
    ...
    assert _is_scheduled_time("standup", datetime(2026, 7, 4, 9, 0))
