...
def test_intent_to_filter_uses_explicit_dates():
    ...
    assert f["$lte"] == datetime(2026, 7, 2, 0, 0, tzinfo=UTC)
...
def test_scope_to_range_windows_are_ordered():
    frozen = datetime(2026, 7, 8, 12, 0, tzinfo=UTC)
    ...
    assert today == datetime(2026, 7, 8, 0, 0, tzinfo=UTC)
    assert week  == datetime(2026, 7, 6, 0, 0, tzinfo=UTC)
    assert month < week <= today
    assert allt == today


def test_claude_date_range():
    tf = {"$gte": datetime(2026, 7, 6, 7, 0, tzinfo=UTC),
          "$lte": datetime(2026, 7, 9, 7, 0, tzinfo=UTC)}
    lo, hi = q._claude_date_range(tf, LA)
    assert lo == "2026-07-06"
    assert hi == "2026-07-08"
