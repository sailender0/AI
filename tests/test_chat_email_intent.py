...
def test_md_html_escapes_and_formats():
    ...
    assert "<script>" not in h and "&lt;script&gt;" in h   # escaped
    assert h.count("<li>") == 2                             # bullets → list items
    assert "<b>bold</b>" in h                               # **bold** → <b>
...
def test_render_chat_uses_report_shell():
    ...
    assert "Developer Activity Tracker" in html            # the shared _wrap footer
